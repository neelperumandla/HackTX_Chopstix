...
def get_contours(frame):
    ...
    frame_threshold = cv2.erode(
        mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2, 2))
    )
    frame_threshold = cv2.dilate(
        frame_threshold, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2, 2))
    )
...
def direction(frame):
    ...
    tgt = find_closest_bbox(
        [cv2.boundingRect(contour) for contour in bounding_box],
        cv2.boundingRect(bounding_box[0]),
    )
